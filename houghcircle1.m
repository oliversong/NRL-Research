function [y0detect,x0detect,Accumulator] = houghcircle1(Imbinary,ri,w,thresh,region)
...
if nargin > 3 & thresh < 4
    error('treshold value must be bigger or equal to 4');
    return
end
if w > ri
    error('w must be less than r');
    return
end
if nargin < 4 thresh = 4; end
...
for cnt = 1:length(xIndex) 
 for i=0:w-1  % loop through the band
    r=ri-i;
