function [y0detect,x0detect,Accumulator] = houghcircle(Imbinary,r,w,thresh)
...
if r < w
    error('r value must be bigger or equal to w');
end
...
for cnt = 1:length(xIndex)
    ...
    for x0 = low:high
      for i = 0:w-1
        y01 = yIndex(cnt)-sqrt((r-i)^2-(xIndex(cnt)-x0)^2);
        y02 = yIndex(cnt)+sqrt((r-i)^2-(xIndex(cnt)-x0)^2);
